...
public class EquipoController {
    ...
    public EquipoController(EquipoService service, DependenciaService depService,
        ...
        this.service = service; this.depService = depService; this.audit = audit;
        this.form = form; this.tm = tm; this.table = table;
        ...
        form.onGuardar(() -> {
            try {
                ...
                tm.setData(service.listar());
                form.clearAndFocus();
                audit.log(new AuditEntry(Session.operador(), "Guardar Equipo", "ID="+e.getId()+" COD="+e.getCodigo()));
                JOptionPane.showMessageDialog(form,"Guardado","Info",JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception ex){ JOptionPane.showMessageDialog(form,ex.getMessage(),"Error",JOptionPane.ERROR_MESSAGE); }
        });
        ...
        form.onEliminar(id -> {
            ...
            } else JOptionPane.showMessageDialog(form,"No se encontró el ID","Aviso",JOptionPane.WARNING_MESSAGE);
        });
        ...
        form.onFiltrar(() -> {
            ...
            if (depId!=null) data = service.listarPorDependencia(depId);
            if (t!=null) {
                var porTipo = service.listarPorTipo(t);
                // si aplicaste dep también, intersección simple:
                if (depId!=null) {
                    var ids = porTipo.stream().map(x->x.getId()).collect(java.util.stream.Collectors.toSet());
                    data = data.stream().filter(x -> ids.contains(x.getId())).toList();
                } else {
                    data = porTipo;
                }
            }
    ...
    private void cargarDependencias(EquipoForm form, EquipoTableModel tm){
        ...
        var opciones = deps.stream().map(d -> new DepOpcion(d.getId(), d.getNombre())).toList();
    ...
    private void aplicarPermisosPorRol(){
        ...
        // (obtenemos referencias por reflexión ligera desde el form si fuera necesario;
        // acá no hace falta: ya están encapsulados en listeners; simplificamos no tocando más)
        // Sugerencia práctica:
        // - Para LECTURA: deshabilitá Guardar y Eliminar con enable(false)
        // - Para EDICION: deshabilitá solo Eliminar
        // - Para ADMIN: todo habilitado
        // Lo hacemos con inspec de componentes:
        for (var comp : form.getComponents()){
            // no reliable; mejor mostrar un diálogo: (APB)
        }
        // APB: notificamos al usuario el rol activo (para la corrección):
        JOptionPane.showMessageDialog(form,"Rol activo: "+r+
                (puedeEditar? " (edita)" : " (solo lectura)")+
                (puedeBorrar? ", puede borrar" : ""), "Permisos", JOptionPane.INFORMATION_MESSAGE);
    }
